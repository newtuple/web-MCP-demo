...
const AVIATION_SIGNALS = [
  'Querying cargo, baggage, and delay systems in parallel',
  'Synthesizing operational answers in plain language',
  'Generating live visuals from current flight data',
  'Maintaining governance-ready action traces',
]

export default function AviationContent({ data }: { data: AviationData }) {
  ...
  const selectedCapability = capabilities[Math.min(activeCapability, Math.max(capabilities.length - 1, 0))]
  ...
  return (
    <>
          ...
          <FadeIn>
            <div className="max-w-5xl mx-auto text-center">
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-4 mb-8">
                ...
                <Button href="#aviation-capabilities" variant="outline" size="lg" className="border-gray-300 text-gray-900 hover:text-white" fillClassName="bg-[var(--accent-900)]">
                  Explore capabilities
                </Button>
              </div>
              <StaggerChildren className="grid grid-cols-1 md:grid-cols-2 gap-2.5 max-w-3xl mx-auto">
                {AVIATION_SIGNALS.map((signal) => (
                  <StaggerItem key={signal}>
                    <article className="group relative overflow-hidden rounded-xl border border-gray-200 bg-white px-3 py-2 text-xs md:text-sm text-gray-600 font-light transition-all duration-300 hover:border-[var(--accent-300)] hover:bg-[var(--accent-50)]/35">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.08fr_0.92fr] gap-6 md:gap-8">
            <FadeIn delay={0.05}>
              <article className="group relative h-full overflow-hidden rounded-3xl border border-gray-800/90 bg-gray-900/90 p-5 md:p-6 shadow-[0_20px_40px_-28px_rgba(0,0,0,0.65)]">
                <div className="pointer-events-none absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-transparent via-[var(--accent-400)] to-transparent -translate-x-[110%] transition-transform duration-700 group-hover:translate-x-0" />
                <span className="inline-flex items-center rounded-full border border-[var(--accent-500)]/35 bg-[var(--accent-500)]/15 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--accent-300)] mb-4">
                  {data.caseStudy.badge}
                </span>
                <div className="space-y-3.5">
                  {data.caseStudy.bullets.map((bullet) => (
                    <div key={bullet} className="flex items-start gap-2.5">
                      <CheckCircle2 className="h-[18px] w-[18px] text-[var(--accent-300)] mt-0.5 flex-shrink-0" strokeWidth={1.6} />
            ...
            <FadeIn delay={0.1}>
              <article className="group relative h-full overflow-hidden rounded-3xl border border-gray-800/90 bg-gray-900/90 p-5 md:p-6 shadow-[0_20px_40px_-28px_rgba(0,0,0,0.65)]">
                <div className="pointer-events-none absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-transparent via-[var(--accent-400)] to-transparent -translate-x-[110%] transition-transform duration-700 group-hover:translate-x-0" />
                <div className="flex h-11 w-11 items-center justify-center rounded-xl border border-[var(--accent-500)]/35 bg-[var(--accent-500)]/15 mb-4">
                  <Plane className="h-5 w-5 text-[var(--accent-300)]" strokeWidth={1.6} />
                </div>
                ...
                <p className="text-sm text-gray-500 uppercase tracking-[0.12em] font-semibold">
                  Flight Operations Outcome
                </p>
                <p className="text-sm text-gray-400 font-light mt-1 leading-relaxed">
                  Faster answers, reduced dependence on specialist query teams, and higher operational confidence.
                </p>
              </article>
            </FadeIn>
          </div>
      ...
      <Section id="aviation-capabilities" className="py-14 md:py-16 bg-white">
        <FadeIn>
          <div className="text-center mb-10 md:mb-12">
            <p className="text-[11px] uppercase tracking-[0.16em] text-[var(--accent-900)] font-semibold mb-2">
              Aviation Stack
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              {data.capabilities.sectionTitle}
            </h2>
          </div>
        </FadeIn>
