...
function AcceleratorsHeroVisual({ items }: { items: AcceleratorViewItem[] }) {
  return (
    <div className="relative mx-auto w-full max-w-[560px] px-1">
      ...
      <div className="group relative min-h-[360px] sm:min-h-[410px] md:min-h-[440px] overflow-hidden rounded-3xl border border-[var(--accent-100)]/90 bg-white/85 p-4 sm:p-5 md:p-6 shadow-[0_32px_68px_-42px_rgba(0,71,171,0.5)] backdrop-blur-sm">
            ...
            {items.slice(0, 4).map((item) => (
              ...
              >
                <div className="mb-2 flex items-center gap-2.5">
                  <div className="h-7 w-7 rounded-lg border border-[var(--accent-100)] bg-[var(--accent-50)] flex items-center justify-center">
                    {item.name.toLowerCase() === 'dialogtuple' ? (
                      ...
                      <item.icon className="h-3.5 w-3.5 text-[var(--accent-900)]" strokeWidth={1.5} />
                    )}
                  </div>
    ...
    <>
      <Hero
        ...
        title={
          <>
            ...
            <span className="mt-2 block text-[var(--accent-900)] font-semibold tracking-tight">Build faster, ship with confidence</span>
          </>
        }
      ...
      >
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <Button href="#accelerator-suite" variant="outline" size="lg" className="border-gray-300 text-gray-900 hover:text-white" fillClassName="bg-[var(--accent-900)]">
            Explore suite
          </Button>
        </div>
      </Hero>
        ...
        <StaggerChildren className="space-y-3 max-w-5xl mx-auto">
          {whyItems.map((item, index) => (
            <StaggerItem key={item.title}>
              <article className="group relative overflow-hidden rounded-2xl border border-gray-200/90 bg-white px-4 py-4 md:px-6 md:py-5 shadow-[0_14px_26px_-20px_rgba(15,23,42,0.25)] transition-all duration-300 hover:border-[var(--accent-300)] hover:shadow-[0_24px_42px_-26px_rgba(0,71,171,0.38)]">
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-5">
                  <div className="flex items-center gap-3 md:w-[290px]">
                    ...
                    <div className="h-10 w-10 rounded-xl border border-[var(--accent-100)] bg-[var(--accent-50)] flex items-center justify-center">
                      <item.icon className="h-5 w-5 text-[var(--accent-900)]" strokeWidth={1.5} />
                    </div>
                ...
                {acceleratorItems.map((item, index) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5 mb-1.5">
                      <div className="h-8 w-8 rounded-lg border border-[var(--accent-100)] bg-[var(--accent-50)] flex items-center justify-center">
                        {item.name.toLowerCase() === 'dialogtuple' ? (
                          <span className="inline-flex items-center justify-center rounded-md bg-white p-0.5 shadow-[0_8px_14px_-10px_rgba(0,71,171,0.35)]">
                            <Image
                              src={DIALOGTUPLE_LOGOS.darkIcon}
              ...
              <article className="rounded-3xl border border-gray-200/90 bg-white shadow-[0_20px_40px_-28px_rgba(15,23,42,0.32)] overflow-hidden">
                <div className="border-b border-gray-200 bg-gradient-to-r from-[var(--accent-50)]/50 via-white to-cyan-50/35 p-5 md:p-6">
                  ...
                  {selectedAccelerator.name.toLowerCase() === 'dialogtuple' && (
                    <span className="mb-2.5 inline-flex items-center rounded-xl border border-[var(--accent-200)] bg-white px-3 py-1.5">
                      <Image
                  ...
                  <p className="text-sm md:text-base text-[var(--accent-900)] font-medium mb-2.5">{selectedAccelerator.tagline}</p>
                ...
                <div className="p-5 md:p-6 lg:p-7">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 mb-6">
                    {acceleratorItems.map((item) => (
                      <Link
                        key={item.name}
                        href={item.href}
                        className="group relative overflow-hidden rounded-2xl border border-gray-200/90 bg-gray-50/70 p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-[var(--accent-300)] hover:bg-white"
                      >
                        <div className="pointer-events-none absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-transparent via-[var(--accent-400)] to-transparent -translate-x-[110%] transition-transform duration-700 group-hover:translate-x-0" />
                  ...
                  <Button href={selectedAccelerator.href} size="md" className="bg-gray-950 text-white hover:text-white" fillClassName="bg-[var(--accent-900)]">
      ...
      <Section className="py-14 md:py-16 bg-white">
        ...
        <StaggerChildren className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6 max-w-4xl mx-auto">
          {licensingItems.map((item, index) => (
            <StaggerItem key={item.title}>
              <article className="group relative h-full min-h-[220px] overflow-hidden rounded-2xl border border-gray-200/90 bg-white p-5 md:p-6 shadow-[0_14px_28px_-22px_rgba(15,23,42,0.3)] transition-all duration-300 hover:-translate-y-1 hover:border-[var(--accent-300)] hover:shadow-[0_24px_42px_-26px_rgba(0,71,171,0.4)]">
                <div className="pointer-events-none absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-transparent via-[var(--accent-400)] to-transparent -translate-x-[110%] transition-transform duration-700 group-hover:translate-x-0" />
        ...
        <FadeIn delay={0.2}>
          <div className="mt-8 rounded-2xl border border-[var(--accent-100)] bg-[var(--accent-50)]/45 p-5 md:p-6 max-w-4xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
              <div className="h-10 w-10 rounded-xl border border-[var(--accent-200)] bg-white flex items-center justify-center">
                <Headphones className="h-5 w-5 text-[var(--accent-900)]" strokeWidth={1.8} />
              </div>
