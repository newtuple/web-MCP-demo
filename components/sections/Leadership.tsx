    ...
    <Section id="leadership" className="py-14 md:py-16 bg-gray-50">
      <FadeIn>
        <div className="mx-auto mb-10 max-w-3xl text-center md:mb-12">
          <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--accent-900)]">
            {eyebrow}
          </p>
        ...
        {members.map((member) => {
          ...
          return (
            <StaggerItem key={member.name} className="h-full">
              <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-gray-200/90 bg-white px-5 pb-6 pt-5 text-center shadow-[0_16px_32px_-24px_rgba(15,23,42,0.32)] transition-all duration-300 hover:-translate-y-1 hover:border-[var(--accent-300)] hover:shadow-[0_24px_42px_-26px_rgba(0,71,171,0.42)]">
                <div className="pointer-events-none absolute inset-x-0 top-0 h-[2px] -translate-x-[110%] bg-gradient-to-r from-transparent via-[var(--accent-400)] to-transparent transition-transform duration-700 group-hover:translate-x-0" />
                ...
                <a
                  ...
                  className="absolute right-3.5 top-3.5 inline-flex h-8 w-8 items-center justify-center rounded-lg border border-[var(--accent-100)] bg-[var(--accent-50)] text-[var(--accent-900)] transition-colors hover:border-[var(--accent-300)] hover:bg-[var(--accent-100)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-500)] focus-visible:ring-offset-2"
                >
                  <Linkedin className="h-4 w-4" strokeWidth={1.8} />
                </a>

                <div className="mx-auto mb-5 rounded-full bg-gradient-to-br from-[var(--accent-200)] via-[var(--accent-50)] to-cyan-100 p-1.5">
                  <div className="relative h-36 w-36 overflow-hidden rounded-full bg-white sm:h-40 sm:w-40 lg:h-36 lg:w-36 xl:h-40 xl:w-40">
                    {member.image ? (
                      ...
                      <div
                        aria-hidden="true"
                        className="flex h-full w-full items-center justify-center bg-[var(--accent-50)] text-4xl font-semibold tracking-tight text-[var(--accent-900)]"
                      >
                        {initials}
                      </div>
                    )}
                ...
                <div className="flex min-h-[88px] flex-col items-center">
                  ...
                  <p className="flex min-h-[40px] items-start justify-center text-sm font-semibold leading-snug text-[var(--accent-900)]">
                    {member.role}
                  </p>
                </div>
